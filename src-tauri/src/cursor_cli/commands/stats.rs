...
pub fn get_stats() -> Result<CursorStats, String> {
    ...
    Ok(CursorStats {
        ...
        // cursor_cli never reaches the cursor.com API, so "ok" reflects that
        // there's nothing to authenticate — the absence of token figures is by
        // design (Cursor CLI stores no token counts locally either).
